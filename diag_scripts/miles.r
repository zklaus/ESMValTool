...
for (model_idx in c(1:(length(models_name)-1))) {
    ...
    year2=models_end_year[model_idx]
    for (seas in seasons) {
       miles.block.figures( year1=year1, year2=year2, exp=exp, dataset_ref=dataset_ref, year1_ref=year1_ref, year2_ref=year2_ref, season=seas,FIGDIR=work_dir,FILESDIR=work_dir,REFDIR=work_dir,CFGSCRIPT=diag_script_cfg)
    }
}
