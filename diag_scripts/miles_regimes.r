...
source('diag_scripts/aux/miles/regimes_fast.R')
source('diag_scripts/aux/miles/regimes_figures.R')
...
for (model_idx in c(1:(length(models_name)))) {
    ...
    for (seas in seasons) {
       miles.regimes.fast( year1=year1, year2=year2, exp=exp, season=seas,z500filename=zdirfile,FILESDIR=work_dir,nclusters=nclusters)
    }
}
